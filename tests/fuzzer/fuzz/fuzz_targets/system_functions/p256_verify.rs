...
fn fuzz(data: &[u8]) {
    ...
    let src = u.arbitrary::<[u8; P256_INPUT_REQUIRED_LENGTH]>().unwrap();
    ...
    let n = u
        .arbitrary::<u8>()
        .unwrap_or(P256_INPUT_REQUIRED_LENGTH as u8) as usize;
    if n > P256_INPUT_REQUIRED_LENGTH {
        return;
    }
    ...
    let _ = P256VerifyImpl::execute(&src.as_slice()[0..n], &mut dst, &mut resource, allocator);
}
