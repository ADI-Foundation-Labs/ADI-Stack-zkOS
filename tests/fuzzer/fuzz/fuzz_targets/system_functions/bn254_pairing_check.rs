...
const BN254_ECPAIRING_SRC_REQUIRED_LENGTH: usize = 192;

fn fuzz(data: &[u8]) {
    let u = &mut Unstructured::new(data);
    let src = u
        .arbitrary::<[u8; 2 * BN254_ECPAIRING_SRC_REQUIRED_LENGTH]>()
        .unwrap();
    ...
    let n = u
        .arbitrary::<u16>()
        .unwrap_or(2 * BN254_ECPAIRING_SRC_REQUIRED_LENGTH as u16) as usize;
    if n > 2 * BN254_ECPAIRING_SRC_REQUIRED_LENGTH {
        return;
    }
    ...
    let _ =
        Bn254PairingCheckImpl::execute(&src.as_slice()[0..n], &mut dst, &mut resource, allocator);
}
