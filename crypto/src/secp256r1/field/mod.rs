...
impl FieldElementConst {
    ...
    pub(super) const fn into_fe(self) -> FieldElement {
        self
    }
...
mod tests {
    ...
    fn montgomerry_repr_round() {
        ...
        proptest!(|(x: FieldElement)| {
            prop_assert_eq!(x.clone().into_integer().into_representation(), x.clone());
            prop_assert_eq!(x.clone().into_representation().into_integer(), x);
        })
    }
    ...
    fn bytes_round() {
        ...
        proptest!(|(bytes: [u8; 32])| {
            if let Ok(x) = FieldElement::from_be_bytes(&bytes) {
                prop_assert_eq!(x.into_be_bytes(), bytes);
            }
        });

        proptest!(|(x: FieldElement)| {
            prop_assert_eq!(FieldElement::from_be_bytes(&x.clone().into_be_bytes()).unwrap(), x);
        });
    }
}
