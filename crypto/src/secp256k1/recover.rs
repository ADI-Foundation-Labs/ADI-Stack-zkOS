...
pub fn recover_with_context(
    ...
    let mut brx = sigr.clone().into_repr();
    ...
    let mut pk = ecmult(xj, sigs, sigr, context).into_affine();
...
mod tests {
    ...
    fn ecmult_1G_0G() {
        ...
        let res = ecmult(
            Affine::GENERATOR.into_jacobian(),
            Scalar::ONE,
            Scalar::ZERO,
            &ECRECOVER_CONTEXT,
        )
    ...
    fn ecmult_1G_1G() {
        ...
        let mut expected = Affine::GENERATOR.into_jacobian();
        expected.double_in_place(None);

        assert_eq!(res, expected.into_affine())
    }
    ...
    fn test_generator_multipules() {
        for (k, x, y) in MUL_TEST_VECTORS {
            ...
            let computed_ctx =
                ecmult(Jacobian::INFINITY, Scalar::ZERO, k.clone(), &ECRECOVER_CONTEXT).into_affine();
