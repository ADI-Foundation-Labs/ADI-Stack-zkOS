...
- [`zksync_os_runner`](../zksync_os_runner/src/lib.rs) defines a `run` function that takes the RISC-V binary and the initial oracle (non-determinism source) and simulates the execution of that binary. This is done using our [`risc_v_simulator`](https://github.com/ADI-Foundation-Labs/ADI-Stack-Airbender-System/tree/main/risc_v_simulator).  This is the method called by the testing rig for the proof run.
